...
const EditCar = () => {
  ...
  const handleSaveDetails = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Failed to update vehicle details.');
    } finally {
  ...
  const handleDocumentUpload = async (docType, e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        alert(err.response?.data?.message || `Failed to upload ${docType} document.`);
      } finally {
  ...
  const handleDeleteDocument = async (docType) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || `Failed to delete ${docType} document.`);
    } finally {
      setUploadingDoc(null);
    }
  };
