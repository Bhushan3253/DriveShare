...
public class EmailService {
    ...
    @Value("${brevo.sender.email:no-reply@driveshare.com}")
    ...
    public void sendVerificationEmail(String toEmail, String userName, String rawToken) {
        ...
        String textContent = "Welcome to DriveShare!\n\nPlease verify your email address by copying and opening this link in your browser:\n"
                + verificationUrl + "\n\nThis link is valid for 30 minutes.\n\nBest regards,\nDriveShare Operations Team";

        // 1. Primary: Brevo HTTP API (Port 443 HTTPS - Works across cloud networks)
        if (brevoApiKey != null && !brevoApiKey.trim().isEmpty()) {
            try {
                if (sendViaBrevo(toEmail, userName, subject, htmlContent, textContent)) {
                    log.info("✓ [BREVO API] Verification email sent successfully to: {}", toEmail);
                    return;
                }
            } catch (Exception e) {
                log.warn("⚠️ Brevo HTTP API delivery failed to {}: {}. Attempting next provider.", toEmail, e.getMessage());
            }
        }

        // 2. Secondary: Resend HTTP API (Port 443 HTTPS - Works on Render/AWS/GCP/Vercel)
        if (resendApiKey != null && !resendApiKey.trim().isEmpty()) {
            try {
                if (sendViaResend(toEmail, subject, htmlContent, textContent)) {
                    log.info("✓ [RESEND API] Verification email sent successfully to: {}", toEmail);
                    return;
                }
            } catch (Exception e) {
                log.warn("⚠️ Resend HTTP API delivery failed to {}: {}. Attempting standard SMTP.", toEmail, e.getMessage());
            }
        }

        // 3. Tertiary: JavaMailSender (SMTP)
        if (mailSender != null && mailUsername != null && !mailUsername.trim().isEmpty()) {
            try {
                ...
                log.info("✓ [SMTP] Verification email sent successfully to: {}", toEmail);
                return;
            } catch (Exception e) {
